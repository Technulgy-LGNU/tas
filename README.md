...
The existing `[cloudflare]` values are used by the backend. `images_account_id` is the Cloudflare account ID, while `images_delivery_url` is the delivery base including the account hash, for example `https://imagedelivery.net/<account_hash>`. Set `images_variant` to an existing public source variant, such as `public`. Image URLs now request resizing and quality 80 (website width 1920, admin previews 640). For explicit WebP conversion through `/cdn-cgi/image/`, set `images_transform_origin` to your Cloudflare-proxied HTTPS origin after enabling Image Transformations for its zone and allowing the delivery host as a source. Without that setting, hosted flexible variants still negotiate format using the request’s `Accept` header. See [image delivery setup](docs/website-api.md) for configuration and verification. The API token needs **Images Write** permission for that account. Never expose this token to the frontend.
