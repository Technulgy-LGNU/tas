...
The existing `[cloudflare]` values are used by the backend. `images_account_id` is the Cloudflare account ID, while `images_delivery_url` is the delivery base including the account hash, for example `https://imagedelivery.net/<account_hash>`. Enable Flexible variants under Cloudflare Hosted Images → Delivery. Image URLs request resizing and quality 80 (website width 1920, admin previews 640). Optionally set `images_transform_origin` to a Cloudflare-proxied HTTPS origin in the same account to use native `/cdn-cgi/imagedelivery/` delivery. This avoids the nested remote transformation that caused error 9524. Hosted delivery negotiates format using the request’s `Accept` header; use `Accept: image/webp` for server-side WebP downloads. The legacy `images_variant` setting is no longer used. See [image delivery setup](docs/website-api.md) for configuration and verification. The API token needs **Images Write** permission for that account. Never expose this token to the frontend.
