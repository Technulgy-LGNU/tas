...
func TestDeliveryURLs(t *testing.T) {
	for _, tc := range []struct {
		...
		{"explicit transform", "https://imagedelivery.net/hash/", "tas-photo", "https://images.example.org/", "https://images.example.org/cdn-cgi/image/width=1920,fit=scale-down,quality=80,format=webp/https://imagedelivery.net/hash/tas-photo/public"},
		{"custom delivery and escaped ID", "https://images.example.org/cdn-cgi/imagedelivery/hash", "folder/photo?x#y", "https://www.example.org", "https://www.example.org/cdn-cgi/image/width=1920,fit=scale-down,quality=80,format=webp/https://images.example.org/cdn-cgi/imagedelivery/hash/folder%2Fphoto%3Fx%23y/public"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if got := DeliveryURL(tc.base, tc.id, "public", tc.origin, 1920); got != tc.want {
